...
The **angular-dashboard** name used in above commands is only example image name.
...
`docker run --name angular-dashboard -p 8080:80 angular-dashboard &`
...
If you want to run image as virtual-host on sub-domain you must setup [proxy](https://github.com/jwilder/nginx-proxy). You should install proxy and set sub-domain in this way:
 ...
 And in your `/etc/hosts` file (linux) add line: `127.0.0.1 angular-dashboard.your-domain.com` or in yor hosting add
...
```
docker run -e VIRTUAL_HOST=angular-dashboard.your-domain.com --name angular-dashboard angular-dashboard &
```

### Login into docker container

`docker exec -t -i angular-dashboard /bin/bash`
